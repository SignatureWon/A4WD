...
<h3 class="h3 mt-4">Payment Schedule</h3>
...
{/each}
<div class="bg-brand-100 p-4 border border-brand-500 mt-4">
...
<h3 class="h3 mt-8">Payments</h3>
{#each quote.payments as item, index}
  <div class="py-2 border-b border-gray-200">
    <div class="flex">
      <div class="w-28">{item.date}</div>
      <div class="flex-1">
        <div>{item.method}</div>
        <div class="text-gray-400">{@html item.remark || "&nbsp;"}</div>
      </div>
      <div class="text-right w-16">${format.currency(item.amount)}</div>
      <div>
        <Button
          kind="danger-ghost"
          size="small"
...
<div class="border border-gray-200 mt-4">
  <div class="p-4">
    <h3 class="h3 mb-4">Credit Card Details</h3>
