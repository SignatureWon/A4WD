    ...
    {#each value as row, rowIndex}
      <div class="flex pb-4 mb-4 border-b border-gray-200">
        ...
        <div class="flex-1">
          <div class="md:flex">
            <div class="md:flex-1">
              <TextInput bind:value={row.name} placeholder="Name" />
              <TextArea
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="grid grid-cols-2 md:grid-cols-1 gap-4">
                  <NumberInput
                    label="Nett Rate"
                    bind:value={row.nett_rate}
                    step={0.01}
                  />
                  <NumberInput
                    label="Nett Cap"
                    bind:value={row.nett_cap}
                    step={0.01}
                  />
                </div>
                <div class="grid grid-cols-2 md:grid-cols-1 gap-4">
                  <NumberInput
                    label="Gross Rate"
                    bind:value={row.gross_rate}
                    step={0.01}
                  />
                  <NumberInput
                    label="Gross Cap"
                    bind:value={row.gross_cap}
                    step={0.01}
                  />
                </div>
                <div class="grid grid-cols-2 md:grid-cols-1 gap-4">
                  <NumberInput label="Max Qty" bind:value={row.qty} step={1} />
                  <div class="grid grid-cols-2">
                    <Toggle labelText="Daily" bind:toggled={row.daily} />
                    <Toggle
                      labelText="Inclusive"
                      bind:toggled={row.inclusive}
                    />
                  </div>
                </div>
              </div>
