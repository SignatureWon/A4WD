<script>
  ...
  const duration = data.quote.details.duration;
  const date_quote = dayjs(data.quote.created_at).format("DD MMM YYYY");
  const date_start = dayjs(data.quote.details.date_start).format(
    "ddd, DD MMM YYYY"
  );
  const date_end = dayjs(data.quote.details.date_end).format(
    "ddd, DD MMM YYYY"
  );
  let agentFees = [];
  let supplierFees = [];
  ...
  const getDailyRates = () => {
    let obj = data.quote.details.daily;
    let type = data.quote.details.rates_type;
    let arr = obj.items;
    if (type === "flex") {
      let week = 1;
      let day = 0;
      arr.forEach((o, i) => {
        if (i !== 0 && i % 7 === 0) {
          agentFees.push({
            name: `Daily basic rental: Week ${week}: Flex[${
              arr[i - 1].flex
            }]: $${format.currency(arr[i - 1].gross)} x ${day} days`,
          ...
          //   week: week,
          //   day: day,
          //   start: dayjs(arr[i - 1].day),
          //   end: dayjs(arr[i - 1].day).add(6, "day"),
          //   gross: arr[i - 1].gross,
          //   flex: arr[i - 1].flex,
          //   total: arr[i - 1].gross * day,
          // });
        ...
        if (i === arr.length - 1) {
          ...
          agentFees.push({
            name: `Daily basic rental: Week ${week}: Flex[${
              o.flex
            }]: $${format.currency(o.gross)} x ${day} days`,
            total: o.gross * day,
            nett: o.nett * day,
            profit: o.profit * day,
          });
      ...
      agentFees.push({
        name: `Daily basic rental: $${format.currency(
          obj.gross / arr.length
        )} x ${arr.length} days`,
        total: obj.gross,
        nett: obj.nett,
        profit: obj.profit,
      ...
      //     week: null,
      //     day: arr.length,
      //     start: dayjs(arr[0].day),
      //     end: dayjs(arr[arr.length - 1].day),
      //     gross: obj.gross / arr.length,
      //     flex: null,
      //     total: obj.gross,
      //   },
      // ];
  ...
  const getBonds = () => {
    const bond = Object.keys(data.quote.details.bonds).length
      ? data.quote.details.bonds
      : data.quote.details.bond;
    ...
    supplierFees.push({
      name: `Bond: $${format.currency(
        bond.bond,
        0
      )} is taken from the hirer's credit or debit card`,
      total: gross,
      nett: nett,
      profit: profit,
    });
  ...
  const getOneways = () => {
    let one_way = data.quote.details.one_way;
    if (one_way > 0) {
      supplierFees.push({
        name: `One-way fee`,
        total: one_way,
  ...
  const getAddons = () => {
    let addons = data.quote.details.addons;
    for (const key in addons) {
      ...
      const row = {
        name: `Add-on: ${addon.name}${
          addon.daily ? `$${addon.gross_rate} x ${duration} days` : ""
        }`,
        total: gross,
        nett: nett,
        profit: gross - nett,
  ...
  const getSpecials = () => {
    let special = data.quote.details.specials;
    if (special.total > 0) {
  ...
  const getFees = () => {
    let fee = data.quote.details.fees;
    if (fee.total > 0) {
      ...
      if (gap < terms.balance) {
        ...
        termsItems = [
          {
            ...
            total: terms.percentage
              ? (total * terms.deposit) / 100
              : terms.deposit,
          },
        ];
        if (terms.payment2) {
          if (terms.balance2 < gap) {
            termsItems.push({
              name: `First payment to agent (${
                ...
                .format("ddd, DD MMM YYYY")} (${terms.balance2} days before
            travel)`,
              total: terms.percentage2
                ? (total * terms.deposit2) / 100
                : terms.deposit2,
            });
          }
        }
        if (terms.payment3) {
          if (terms.balance3 < gap) {
            ...
            travel)`,
              total: terms.percentage3
                ? (total * terms.deposit3) / 100
                : terms.deposit3,
            });
          }
        }
        ...
        if (terms.balance < gap) {
          ...
          termsItems.push({
            name:
              "Balance payment to " +
              (terms.pay_counter
                ? `supplier at pick-up counter on ${date_start.format(
                    "ddd, DD MMM YYYY"
                  )}`
                : `agent on ${dayjs(date_start)
  ...
  const info = {
    ...
    company: {
      name: data.letterhead.name,
      reg: data.letterhead.description,
      contact: data.letterhead.content.replace(/(?:\r\n|\r|\n)/g, "<br>"),
    },
    user: {
      first_name: data.quote.users.first_name,
      last_name: data.quote.users.last_name,
      email: data.quote.users.email,
      phone: data.quote.users.phone,
    },
    quote: {
      id: data.quote.id + 388000,
      date: date_quote,
      duration: duration,
      pickup: {
        name: data.quote.details.pickup.name,
        date: date_start,
      },
      dropoff: {
        name: data.quote.details.dropoff.name,
        date: date_end,
      },
    },
    vehicle: {
    ...
    passenger: {
      adult: data.quote.details.passenger.adult,
      children: data.quote.details.passenger.children,
    },
    driver: {
      age: data.quote.details.driver.age,
      license: data.quote.details.driver.license,
    },
    terms: {
      id: data.quote.details.terms.id,
      confirmation: {
        text: data.quote.details.terms.confirmation_terms,
        pdf: data.quote.details.terms.confirmation,
      },
      summary: {
        text: data.quote.details.terms.summary_terms,
        pdf: data.quote.details.terms.summary,
      },
      counter: {
        text: data.quote.details.terms.counter_terms,
        pdf: data.quote.details.terms.counter,
      },
    },
    daily: getDailyRates(),
    bond: getBonds(),
  ...
  >
    ...
    <tr>
      ...
      >
        ...
        <div style="font-weight: bold">
          {data.quote.users.first_name}
          {data.quote.users.last_name}
        </div>
        <div>
          {data.quote.users.email}
        </div>
        <div>
          {data.quote.users.phone}
        </div>
      </td>
      <td
        width="50%"
      ...
      >
        ...
        <div style="margin-bottom: 20px">
          <a
            href="https://www.australia4wdrentals.com/vehicles/{data.vehicle
              .slug}">View vehicle specs</a
          >
        </div>
        <div>
          <img
            src="https://api.australia4wdrentals.com/storage/v1/render/image/public/contents/{data
